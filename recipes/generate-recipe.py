...
# Generate the recipe json file by running this command inside Factorio's console (open by pressing `)
"""
/c
local out = ""
function write(...)
...
def generate_lean_from_json(json_path: str, lean_path: str):
    ...
    lean_code.append("""structure Recipe where
    name: Option String
    ...
    for name in sorted(recipe_names):
        ...
        print([
            product['amount'] * product.get('probability', 1)
            for product in recipe.get('products', [])
        ])
        print([
            float_to_fraction(product['amount'] * product.get('probability', 1))
            for product in recipe.get('products', [])
        ])
...
if __name__ == "__main__":
    generate_lean_from_json('recipes.json', '../Functorio/Recipe.lean')
